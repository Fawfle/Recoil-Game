...
namespace UnityEngine.UI
{
	...
	public class CustomGridLayoutGroup : LayoutGroup
	{
		...
		[SerializeField] protected Vector2 m_NextLineSpacing = Vector2.zero;

		/// <summary>
		/// The spacing to use between layout elements in the grid on both axises.
		/// </summary>
		public Vector2 nextLineSpacing { get { return m_NextLineSpacing; } set { SetProperty(ref m_NextLineSpacing, value); } }
		...
		private void SetCellsAlongAxis(int axis)
		{
			...
			for (int i = 0; i < rectChildrenCount; i++)
			{
				...
				SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (cellSize[0] + spacing[0]) * positionX + positionY * nextLineSpacing.x, cellSize[0]);
				SetChildAlongAxis(rectChildren[i], 1, startOffset.y + (cellSize[1] + spacing[1]) * positionY + positionX * nextLineSpacing.y, cellSize[1]);
			}
		}
	}
}
